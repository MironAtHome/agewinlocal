...
void vertex_field_cb(void *field, size_t field_len, void *data)
{
    ...
    if (cr->cur_field == cr->alloc)
    {
        ...
        cr->fields_len = (size_t**)repalloc(cr->fields_len, sizeof(size_t) * cr->alloc);
...
void vertex_row_cb(int delim, void *data)
{
    csv_vertex_reader *cr = (csv_vertex_reader*)data;
    agtype *props = NULL;
    size_t i, n_fields;
    ...
    if (cr->row == 0)
    {
        ...
        cr->header = (char **)palloc0((sizeof(char*) * cr->header_num));
        if (cr->header == NULL)
        {
            cr->error = 1;
            ereport(ERROR,
                (errmsg("row_cb: failed to reallocate %zu bytes\n",
                    sizeof(char*) * cr->header_num)));
        }
        cr->header_len = (size_t *)palloc0(sizeof(size_t) * cr->header_num);
        if (cr->header_len == NULL)
        {
            cr->error = 1;
            ereport(ERROR,
                (errmsg("row_cb: failed to reallocate %zu bytes\n",
                    sizeof(size_t) * cr->header_num)));
        }
        for (i = 0; i<cr->header_num; i++)
        {
            ...
            if (cr->header[i] == NULL)
            {
                cr->error = 1;
                ereport(ERROR,
                    (errmsg("row_cb: failed to duplicate string %s\n",
                        cr->fields[i])));
            }
        }
    }
...
int create_labels_from_csv_file(text* file_path,
    char* graph_name,
    Oid graph_oid,
    char* object_name,
    int object_id,
    bool id_field_exists)
{
    ...
    while (true)
    {
        ...
        if (csv_parse(&p, buf->vl_dat, bytes_read, vertex_field_cb,
