...
PGMODULEEXPORT Datum load_labels_from_file(PG_FUNCTION_ARGS)
{
    Name graph_name;
    Name label_name;
    text* file_path;
    ...
    create_labels_from_csv_file(file_path, graph_name_str, graph_oid,
...
PGMODULEEXPORT Datum load_edges_from_file(PG_FUNCTION_ARGS)
{
    ...
    create_edges_from_csv_file(file_path, graph_name_str, graph_oid,
...
PGMODULEEXPORT Datum load_labels_from_table(PG_FUNCTION_ARGS)
{
    Name graph_name;
    Name label_name;
    text* source_table_name_txt;
    List* tableNameSplitList;
    char* graph_name_str;
    char* label_name_str;
    RangeVar* rv;
    char* sep = ".";
    ...
    graph_name = PG_GETARG_NAME(0);
    label_name = PG_GETARG_NAME(1);
    source_table_name_txt = PG_GETARG_TEXT_PP(2);
    id_field_exists = PG_GETARG_BOOL(3);

    graph_name_str = NameStr(*graph_name);
    label_name_str = NameStr(*label_name);

    rv = makeRangeVarFromNameList(textToQualifiedNameList(source_table_name_txt));
    source_table_oid = RangeVarGetRelid(rv, NoLock, true);

    if (!OidIsValid(source_table_oid))
    {
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
            errmsg("could not find valid Oid for table %s", text_to_cstring(source_table_name_txt))));
        PG_RETURN_NULL();
    }
    ...
    create_vertex_from_relationship(text_to_cstring(source_table_name_txt), source_table_oid, graph_name_str, graph_oid,
        label_name_str, label_id, id_field_exists);
    PG_RETURN_VOID();
...
PGMODULEEXPORT Datum load_edges_from_table(PG_FUNCTION_ARGS)
{
    Name graph_name;
    Name label_name;
    ...
    char* graph_name_str;
    char* label_name_str;
    char* source_table_name_str;
    Oid source_schema_oid;
    Oid source_table_oid;
    Oid graph_oid;
    int32 label_id;
    ...
    {
        ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
            errmsg("source table name must not be NULL")));
    }
    ...
    graph_name = PG_GETARG_NAME(0);
    label_name = PG_GETARG_NAME(1);
    source_table_name = PG_GETARG_NAME(2);

    graph_name_str = NameStr(*graph_name);
    label_name_str = NameStr(*label_name);
    source_table_name_str = NameStr(*source_table_name);

    graph_oid = get_graph_oid(graph_name_str);
    label_id = get_label_id(label_name_str, graph_oid);
    source_schema_oid = get_namespace_oid(source_table_name_str, false);
    source_table_oid = get_relname_relid(source_table_name_str, source_schema_oid);

    create_edges_from_relationship(source_table_name_str, source_schema_oid, source_table_oid,
        graph_name_str, graph_oid,
        label_name_str, label_id,
